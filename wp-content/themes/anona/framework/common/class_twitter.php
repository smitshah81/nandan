...
// C U S T O M  T W I T T E R 		
		...
		function relativeTime($a) {
			...
			if(is_numeric($d) && $d > 0) {
				...
				if($d < 3) return __(' right now','iva_theme_front');
				//if less then minute
				if($d < $minute) return floor($d) . __(' seconds ago','iva_theme_front');
				//if less then 2 minutes
				if($d < $minute * 2) return __(' about 1 minute ago','iva_theme_front');
				//if less then hour
				if($d < $hour) return floor($d / $minute) . __(' minutes ago','iva_theme_front');
				//if less then 2 hours
				if($d < $hour * 2) return __(' about 1 hour ago','iva_theme_front');
				//if less then day
				if($d < $day) return floor($d / $hour) . __(' hours ago','iva_theme_front');
				//if more then day, but less then 2 days
				if($d > $day && $d < $day * 2) return __(' yesterday','iva_theme_front');
				//if less then year
				if($d < $day * 365) return floor($d / $day) . __(' days ago','iva_theme_front');
				//else return more than a year
				return __(' over a year ago','iva_theme_front');
			}
		}
		...
		function twitter_parse_cache_feed( $args = '',$twitter_color='') {
