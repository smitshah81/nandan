	...
	function widget( $args, $instance ) {
		...
		echo'<script type="text/javascript">
				jQuery(document).ready(function() {
					"use strict";
					MySlider('.$tm_speed.',"testimonial'.$rand.'");
				});
			</script>';
	...
	while ($testimonial->have_posts()) : $testimonial->the_post();
		...
		switch( $testimonial_image_option ){
			...
			case 'customimage':
							...
							$testimonial_gravatar_image=atp_resize('',$testimonial_image,'40','40','','');
							break;
		}
		$before = $after = '';
		...
		if( $website_url != '' ){
			...
			$clientname = '<strong class="client-name">'.get_the_title().','.$company_name.'</strong>';
		if( $website_name != '' ){
...
$limits = esc_attr( $instance['limits'] );
	...
	?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:','iva_theme_admin'); ?></label> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
		<p>
			<label for="<?php echo $this->get_field_id('Category'); ?>"><?php _e( 'Category:','iva_theme_admin' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('limits'); ?>"><?php _e( 'limits:','iva_theme_admin' ); ?></label> <input type="text" value="<?php echo $limits; ?>" name="<?php echo $this->get_field_name('limits'); ?>" id="<?php echo $this->get_field_id('limits'); ?>" class="widefat" />
			<br />
			<small><?php _e( 'Limits','iva_theme_admin' ); ?></small>
		</p>
<?php
	}
...
add_action('widgets_init', 'wp_widgets_testimonial', 1);
?>