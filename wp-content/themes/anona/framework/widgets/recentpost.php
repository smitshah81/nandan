...
class recentpost_widgets extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_postslist', 'description' => __( "The most recent posts on your site",'iva_theme_admin') );
		parent::__construct('recentpost_entries', __(THEMENAME.'-Recent Posts','iva_theme_admin'), $widget_ops);
			...
			<?php
				if($imagedisable != "true") {
					...
					if ($thumb ){
						...
						?>
						<div class="thumb"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo esc_attr ( get_the_title() ); ?>"> 
				...
				?>
				<div class="pdesc">
				<a href="<?php echo esc_url(get_permalink()) ?>" title="<?php echo esc_attr( get_the_title() ? get_the_title() : get_the_ID() ); ?>"><?php if ( get_the_title() ) the_title(); else the_ID(); ?></a>
...
?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','iva_theme_admin'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of posts to show:','iva_theme_admin' ); ?></label>
		<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>

		<p><input class="checkbox" type="checkbox" <?php checked( $show_date ); ?> id="<?php echo $this->get_field_id( 'show_date','iva_theme_admin' ); ?>" name="<?php echo $this->get_field_name( 'show_date' ); ?>" />
		<label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Display post date?','iva_theme_admin' ); ?></label></p>
		<p><label for="<?php echo $this->get_field_id( 'description_length' ); ?>"><?php _e( 'Length of Description to show:','iva_theme_admin' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id( 'recentpost_imagedisable' ); ?>"><?php _e( 'Disable Post Thumbnail?','iva_theme_admin' ); ?></label></p>
<?php
	}
}
// Register Widget 
