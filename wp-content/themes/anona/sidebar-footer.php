<?php
	 ...
	 */
	$footerwidgetcounts = get_option( 'atp_footerwidgetcount' ) ?  get_option( 'atp_footerwidgetcount' ) : 'half_one_third';
	...
	if ( is_numeric( $footerwidgetcounts ) ) {
		for ( $i = 1; $i <= $footerwidgetcounts; $i++ ) {
			...
			if ( !function_exists( 'dynamic_sidebar' ) || !dynamic_sidebar( 'footercolumn'.$i ) ) : endif;
			echo '</div>';
		}
	} else {
