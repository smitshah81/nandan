<?php
/**
 * Template Name: Cancel Appointments
 */
 get_header();
?>
<div id="primary" class="pagemid">
  <div class="inner">   
    <main class="content-area" role="main">
		...
		if( $iva_appt_action == 'confirm' || $iva_appt_action == 'unconfirm'){
			...
			if( $iva_appt_action == 'unconfirm' ){
				...
				$response = '<div class="iva_message_box success iva-box-solid iva-box-normal"><div class="messagebox_content">'.__('Your Appointment is Unconfirmed', 'iva_theme_front').'</div></div>';
				echo $response;
			}
		
		}
			
		global $default_date,$iva_cancel_templateid;
		...
		//	
		$iva_cncl_apt_limit = get_option('iva_cancel_apt_limit') ?  get_option('iva_cancel_apt_limit') : '-1';
		$args = array(
			...
			'paged' 		=> $paged,
			'posts_per_page'=> $iva_cncl_apt_limit, 
			'orderby'    	=> 'title',
			'order'    		=> 'ASC',
		);
		...
		while( have_posts()):the_post();
			...
			if ( is_super_admin() ) {
				...
				$out .= '<a class="confirm btn green small mr10" '.$disabled.' href="'.esc_url( get_page_link( $iva_cancel_templateid ).'/?email='.urlencode($iva_appointment_email).'&key='.md5($iva_appointment_email).'&action=confirm&id='.get_the_id()).'">'.$iva_appt_confirm_txt.'</a>';
				$out .= '<a class="confirm btn blue small mr10" '.$disabled.' href="'.esc_url( get_page_link( $iva_cancel_templateid ).'/?email='.urlencode($iva_appointment_email).'&key='.md5($iva_appointment_email).'&action=unconfirm&id='.get_the_id()).'">'.$iva_appt_unconfirm_txt.'</a>';
				$out .='</td>';
			}else{
