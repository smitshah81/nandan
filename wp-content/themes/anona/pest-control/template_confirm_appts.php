<?php
/**
 * Template Name: Confirm Appointments
 */
 get_header();
?>
<div id="primary" class="pagemid">
  <div class="inner">   
    <main class="content-area" role="main">
		...
		if( $iva_appt_action == 'unconfirm' || $iva_appt_action == 'cancel'){
			...
			if( $iva_appt_action == 'unconfirm' ){
			
				// Update post status
				update_post_meta( $booking_id,'iva_appt_status','unconfirmed' );
				...
				$response = '<div class="iva_message_box success iva-box-solid iva-box-normal"><div class="messagebox_content">'.__('Your Appointment is Un Confirmed', 'iva_theme_front').'</div></div>';
				echo $response;
			}
		...
		global $default_date,$iva_confirm_templateid;
		...
		$iva_cnfrm_apt_limit = get_option('iva_cnfrm_apt_limit') ?  get_option('iva_cnfrm_apt_limit') : '-1';
		$args = array(
			...
			'paged' 		=> $paged,
			'posts_per_page'=> $iva_cnfrm_apt_limit, 
			'orderby'    	=> 'title',
			'order'    		=> 'ASC',
		);
		...
		$iva_appt_unconfirm_txt		= get_option('iva_unconfirm_txt') ? get_option('iva_unconfirm_txt') : __('UnConfirm','iva_theme_front');
		...
		while( have_posts()):the_post();
			...
			if ( is_super_admin() ) {
				...
				$out .= '<a class="unconfirm btn blue small mr10" '.$disabled.' href="'.esc_url( get_page_link( $iva_confirm_templateid ).'/?email='.urlencode( $iva_appointment_email ).'&key='.md5( $iva_appointment_email ).'&action=unconfirm&id='.get_the_id()).'">'.$iva_appt_unconfirm_txt.'</a>';
				$out .= '<a class="cancel btn red small mr10" '.$disabled.' href="'.esc_url( get_page_link( $iva_confirm_templateid ).'/?email='.urlencode( $iva_appointment_email ).'&key='.md5( $iva_appointment_email ).'&action=cancel&id='.get_the_id()).'" >'.$iva_appt_cancelled_txt.'</a>';
				$out .= '</td>';
			}else{
		...
		if( $iva_count_posts < '1'){
			$out .= '<tr><td colspan="8" align="center">';	
